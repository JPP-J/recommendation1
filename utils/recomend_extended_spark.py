...
# usage
def run_recommendation_analysis_spark(spark, ratings_path: str, frac=0.005, test_size=0.2, n_neighbors=10,
                                n_recommendations=10):
    """
    Run complete recommendation analysis
    """
    ...
    num_rows = df.count()  # Count total rows
    num_cols = len(df.columns)  # Count total columns
    print(f"Shape: ({num_rows}, {num_cols})")
    print(f'Schema: \n{df.printSchema()}')
    print(f"Columns: {df.columns}")
    print(f"Sampling data shape: {df.shape}")
